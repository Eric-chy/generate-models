...
func getTables(tableNames string) []Table {
	...
	var sql string
	if tableNames == "" {
		sql = "SELECT TABLE_NAME as Name,TABLE_COMMENT as Comment FROM information_schema.TABLES WHERE table_schema='" + cfg.DBName + "';"
	} else {
		sql = "SELECT TABLE_NAME as Name,TABLE_COMMENT as Comment FROM information_schema.TABLES WHERE TABLE_NAME IN (" + tableNames + ") AND table_schema='" + cfg.DBName + "';"
	}
